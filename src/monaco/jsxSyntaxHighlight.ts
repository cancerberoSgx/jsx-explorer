...
export function dispatchSyntaxHighlight(data: CodeWorkerResponse) {
  ...
  const decorations: monaco.editor.IModelDeltaDecoration[] = data.jsxSyntaxHighLight.classifications.map(
    classification => {
      ...
      const inlineClassName = `${classification.kind} ${classification.parentKind} ${classification.parentKind} ${(
